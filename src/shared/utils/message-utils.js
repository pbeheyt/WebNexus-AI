...
export async function robustSendMessage(message, attempt = 1) {
  ...
  return new Promise((resolve, reject) => {
    ...
    chrome.runtime.sendMessage(message, (response) => {
      ...
      if (lastError) {
          const isConnectionError = lastError.message?.includes('Port closed') || lastError.message?.includes('Could not establish connection') || lastError.message?.includes('The message port closed');
          ...
          if (isConnectionError && attempt <= MAX_RETRIES) {
              ...
              logger.message.warn(`robustSendMessage: Communication failed for action "${message.action}" after ${attempt} attempts (Max retries reached).`);
              const communicationError = new Error(`Communication failed for action "${message.action}" after ${MAX_RETRIES + 1} attempts.`);
