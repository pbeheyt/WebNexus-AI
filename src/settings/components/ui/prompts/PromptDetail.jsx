...
const PromptDetail = ({ prompt, onEdit, onDelete }) => {
  ...
  const handleSetAsDefault = useCallback(async () => {
    ...
    try {
      ...
      await chrome.storage.local.set({ [STORAGE_KEYS.PROMPTS]: customPrompts });
  ...
  return (
    <div className='prompt-detail bg-theme-surface shadow-sm rounded-lg p-5 border border-theme'>
      ...
      <div className={`prompt-detail-content whitespace-pre-wrap  p-4 rounded-lg bg-theme-hover border border-theme mb-5 text-sm text-theme-secondary select-none overflow-hidden prompt-content-scrollable`}>
