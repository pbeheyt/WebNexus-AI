...
const DataManagementTab = () => {
  ...
  const currentOptionObject = useMemo(() => {
    return (
      DATA_MANAGEMENT_OPTIONS.find((opt) => opt.id === selectedDataType) ||
      DATA_MANAGEMENT_OPTIONS[0]
    );
  }, [selectedDataType]);
  ...
  const executeExport = async () => {
    ...
    try {
      if (id === 'all') {
        ...
        result = await userDataService.exportSingleSetting(
          storageKey,
          fileTypeName
        );
      }
      ...
      if (result.success) {
        ...
        showErrorNotification(
          `${failureMsg}: ${result.error || 'Unknown error'}`
        );
      }
    } catch (err) {
      logger.settings.error(`Export error for ${id}:`, err);
      showErrorNotification(
        `${failureMsg}: ${err.message || 'Unexpected error during export'}`
      );
  ...
  const handleFileSelectedForImport = async (event) => {
    ...
    const { keyForStorage, nameForLoading } =
      currentImportActionDetails.current;
    ...
    try {
      ...
      logger.settings.error(
        `Import file processing error for ${nameForLoading}:`,
        err
      );
      showErrorNotification(
        `Import failed: ${err.message || 'Error processing file'}`
      );
      setIsImportingActual(false);
      result = { success: false, error: err.message };
    }
  ...
  const isAnyOperationLoadingForUI =
    shouldShowExportLoading ||
    shouldShowImportLoading ||
  ...
  const executeReset = async () => {
    ...
    try {
      ...
      logger.settings.error(
        `Reset error for ${dataTypeName} in component:`,
        err
      );
      showErrorNotification(
  ...
  return (
    <div>
      ...
      <p className='section-description text-sm text-theme-secondary mb-6'>
        Manage your extension settings. Select a data type, then choose an
        action.
      </p>
          ...
          <div className='flex flex-col md:flex-row md:items-start md:justify-between'>
            <div className='flex-grow'>
              ...
              <p className='text-sm text-theme-secondary mb-3 md:mb-0'>
                Export the selected data type (
                <strong>{currentOptionObject.name}</strong>) to a JSON file.
                This file can be used later to import these settings.
              </p>
            </div>
            <div className='flex-shrink-0 md:ml-10'>
              ...
              >
                {shouldShowExportLoading
                  ? 'Exporting...'
                  : `Export ${currentOptionObject.name}`}
              </Button>
            </div>
          </div>
        </SettingsCard>
        ...
        <SettingsCard>
          <div className='flex flex-col md:flex-row md:items-start md:justify-between'>
            <div className='flex-grow'>
              ...
              <p className='text-sm text-theme-secondary mb-3 md:mb-0'>
                Import settings from a JSON file for{' '}
                <strong>{currentOptionObject.name}</strong>. This will{' '}
                <strong className='font-semibold'>
                  overwrite existing settings
                </strong>{' '}
                for this data type. The page will reload after a successful
                import.
              </p>
            </div>
            <div className='flex-shrink-0 md:ml-10'>
              ...
              >
                {shouldShowImportLoading
                  ? 'Importing...'
                  : `Import ${currentOptionObject.name}`}
              </Button>
            </div>
          </div>
        </SettingsCard>
        ...
        <SettingsCard>
          <div className='flex flex-col md:flex-row md:items-start md:justify-between'>
            <div className='flex-grow'>
              ...
              <p className='text-sm text-theme-secondary mb-3 md:mb-0'>
                Reset settings for <strong>{currentOptionObject.name}</strong>{' '}
                to their original defaults. The page will reload automatically.
              </p>
            </div>
            <div className='flex-shrink-0 md:ml-10'>
              <Button
              ...
              >
                {shouldShowResetLoading
                  ? 'Resetting...'
                  : `Reset ${currentOptionObject.name}`}
              </Button>
            </div>
          </div>
        </SettingsCard>
