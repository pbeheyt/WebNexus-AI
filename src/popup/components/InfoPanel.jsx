...
export function InfoPanel({ contentTypeLabel, contentType }) { // Add contentType prop
  ...
  // Add style to make the content type icon smaller
  const modifiedIconSvg = iconSvg.replace('w-5 h-5', 'w-3.5 h-3.5');

  return (
    // Add text-center and adjust padding/margins as needed
    <div className="p-2.5 rounded-lg bg-theme-hover/20 dark:bg-theme-hover/10 text-xs text-theme-secondary leading-relaxed">
      <p className="mb-1.5">
        ...
        {/* Conditionally render the icon span with more left/right padding */}
        {modifiedIconSvg && (
          <span
            className="inline-block align-middle mx-1" // Increased horizontal padding
            dangerouslySetInnerHTML={{ __html: modifiedIconSvg }} // Render the modified SVG
          />
        )}
      ...
      <p>
        ...
        {/* Make the panel SVG smaller and add more padding */}
        <svg
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className="w-3.5 h-3.5 inline-block align-text-bottom mx-1 text-theme-primary" // Smaller size and more padding
          stroke="currentColor"
          strokeWidth="2.5"
        >
