...
class GeminiApiService extends BaseApiService {
  ...
  _getGeminiEndpoint(model, method) {
    ...
    if (!model || !method) {
      throw new Error(
        'Model and method are required to build Gemini endpoint.'
      );
    }
    const isPreviewOrExperimental =
      model.includes('-exp-') || model.includes('-preview-');
    const apiVersion = isPreviewOrExperimental ? 'v1beta' : 'v1';
    this.logger.info(
      `[${this.platformId}] Using API version '${apiVersion}' for model '${model}'`
    );
    return baseTemplate
  ...
  async _buildApiRequest(prompt, params, apiKey) {
    const endpoint = this._getGeminiEndpoint(
      params.model,
      ':streamGenerateContent'
    );
    this.logger.info(
      `[${this.platformId}] Building API request to: ${endpoint}`
    );
    ...
    if (params.systemPrompt) {
      if (params.modelSupportsSystemPrompt === true) {
        this.logger.info(
          `[${this.platformId}] Adding system prompt using systemInstruction for model: ${params.model}.`
        );
        ...
        this.logger.warn(
          `[${this.platformId}] System prompts via systemInstruction are not supported by the selected model: ${params.model}. The provided system prompt will be IGNORED.`
        );
      }
    }
    ...
    if (params.tokenParameter) {
      formattedRequest.generationConfig[params.tokenParameter] =
        params.maxTokens;
    } else {
  ...
  _parseStreamChunk(line) {
    if (!line || !line.startsWith('data: ')) {
      if (line === 'data: [DONE]') {
        this.logger.info(
          `[${this.platformId}] SSE stream signal [DONE] received.`
        );
        return { type: 'done' };
      }
      return { type: 'ignore' };
    ...
    try {
      ...
      if (textChunk && typeof textChunk === 'string') {
        ...
        if (data?.error) {
          ...
          this.logger.error(
            `[${this.platformId}] SSE stream returned an error: ${errorMessage}`,
            data.error
          );
          return {
            type: 'error',
        ...
        if (data?.candidates?.[0]?.finishReason) {
          this.logger.info(
            `[${this.platformId}] SSE stream finished with reason: ${data.candidates[0].finishReason}`
          );
        } else {
          this.logger.warn(
            `[${this.platformId}] Parsed SSE data, but no text chunk found or structure mismatch.`,
            data
          );
        }
        return { type: 'ignore' };
      ...
      this.logger.error(
        `[${this.platformId}] Error parsing SSE JSON chunk:`,
        parseError,
        'Raw JSON String:',
        jsonString
      );
  ...
  async _buildValidationRequest(apiKey, model) {
    ...
    this.logger.info(
      `[${this.platformId}] Building validation request to: ${endpoint}`
    );
    const url = new URL(endpoint);
    url.searchParams.append('key', apiKey);
