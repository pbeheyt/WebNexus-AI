...
class GeneralExtractorStrategy extends BaseExtractor {
  constructor() {
    ...
    this.noiseTags = new Set([
      ...
      'NAV',
      'ASIDE',
      'FOOTER', // Often contain less relevant primary content
      'FIGURE',
      'FIGCAPTION', // Can be noisy in some contexts
    ]);
  }
  ...
  _isElementVisible(element) {
    ...
    try {
      ...
      this.logger.warn(
        `Error getting computed style for ${element.tagName}: ${e.message}`
      );
  ...
  _extractVisibleText(node) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      try {
        ...
        /* ignore errors getting style here */
      }
  ...
  async extractAndSaveContent() {
    try {
      this.logger.info(
        'Starting general content extraction (DOM Traversal - Visible Only)...'
      );
  ...
  async extractData() {
    ...
    try {
      ...
      if (selectedText) {
        ...
        this.logger.info(
          'Using selected text as content (moderately cleaned).'
        );
      } else {
        // *** Extract content using DOM traversal for VISIBLE elements ***
        this.logger.info(
          'No selection, extracting visible content using DOM traversal.'
        );
        ...
        try {
          ...
          this.logger.info(
            `DOM traversal extracted and moderately cleaned ${content.length} characters.`
          );
        } catch (domError) {
          this.logger.error(
            'Error during DOM traversal for visible text:',
            domError
          );
          content =
            'Error extracting visible content using DOM traversal: ' +
            domError.message;
          // Fallback to basic text extraction on error
          try {
            this.logger.warn(
              'Attempting fallback to document.body.textContent...'
            );
            ...
            if (fallbackContent) {
              ...
              this.logger.warn(
                `Fallback (all text) extracted and cleaned ${fallbackContent.length} characters after DOM traversal error.`
              );
            } else {
            ...
            this.logger.error(
              'Fallback text extraction failed after DOM traversal error:',
              fallbackError
            );
            content =
              'Content extraction failed completely after DOM traversal error and fallback attempt.';
          }
        }
      }
        ...
        if (authorElement && isVisible(authorElement)) {
          ...
          if (text) {
            ...
            const nameNode =
              ...
              ) || authorElement; // Check itemprop too
            ...
            if (nameNode && isVisible(nameNode)) {
              ...
              if (name) {
                this.logger.info(
                  `Author found in visible element (${selector} -> specific name node)`
                );
                return name;
              }
            }
            ...
            if (name) {
              this.logger.info(
                `Author found in visible element (${selector} -> main element text)`
              );
              return name;
            }
          }
