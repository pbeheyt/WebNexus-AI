...
class GeneralExtractorStrategy extends BaseExtractor {
  constructor(config = {}) {
    ...
    this.logger.info(
      `GeneralExtractorStrategy initialized with mode: ${this.config.extractionMode}`
    );
  }
  ...
  async extractAndSaveContent() {
    try {
      this.logger.info(
        `General content extraction process started (Mode: ${this.config.extractionMode}).`
      );
      const pageData = await this.extractData();
      await this.saveToStorage(pageData);
      this.logger.info(
        'General content extraction process completed and data saved.'
      );
    } catch (error) {
      this.logger.error(
        'Error during general content extraction and saving:',
        error
      );
  ...
  _determineSeparator(prevTextNode, currTextNode) {
    ...
    if (prevParent === currParent) {
      ...
      while (sibling && sibling !== currTextNode) {
        ...
        if (
          ...
          blockTagNames.has(sibling.nodeName.toUpperCase())
        ) {
          return '\n';
        }
    ...
    if (
      blockTagNames.has(prevParent.nodeName.toUpperCase()) ||
      blockTagNames.has(currParent.nodeName.toUpperCase())
    ) {
      return '\n';
    }
    ...
    if (
      prevDisplay === 'block' ||
      currDisplay === 'block' ||
      prevDisplay === 'list-item' ||
      currDisplay === 'list-item'
    ) {
      return '\n';
    }
    ...
    if (
      prevParent.nextElementSibling === currParent &&
      prevParent.nextSibling &&
      prevParent.nextSibling.nodeName === 'BR'
    ) {
      return '\n';
    }
    if (
      prevParent.nextSibling &&
      prevParent.nextSibling.nodeName === 'BR' &&
      prevParent.nextSibling.nextSibling === currParent
    ) {
      return '\n';
    }
  ...
  async _extractBroadContentTreeWalker() {
    ...
    if (textNodes.length === 0) {
      this.logger.warn(
        '_extractBroadContent (TreeWalker) found no text nodes.'
      );
    ...
    for (const currentTextNode of textNodes) {
      ...
      if (previousTextNode === null) {
        ...
        const separator = this._determineSeparator(
          previousTextNode,
          currentTextNode
        );
        if (separator === '\n') {
    ...
    if (normalizedBroadText.length < MIN_BROAD_TEXT_LENGTH) {
      this.logger.warn(
        `_extractBroadContent (TreeWalker) resulted in very little text: ${normalizedBroadText.length} chars.`
      );
      return this._fallbackToReadability(
        'TreeWalker resulted in insufficient text.'
      );
    }

    this.logger.info(
      `_extractBroadContent (TreeWalker) extracted normalized text length: ${normalizedBroadText.length} chars.`
    );
  ...
  async _fallbackToReadability(reason) {
    ...
    if (
      article &&
      article.textContent &&
      article.textContent.trim().length >= MIN_BROAD_TEXT_LENGTH
    ) {
      this.logger.info(
        `Fallback to Readability successful. Length: ${article.textContent.trim().length}`
      );
      return normalizeText(this._moderateCleanText(article.textContent));
    }
    this.logger.warn(
      'Fallback to Readability also yielded insufficient content.'
    );
