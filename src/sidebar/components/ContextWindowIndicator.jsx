...
function ContextWindowIndicator({ contextStatus }) {
  // Always display the component, even with no warning level
  ...
  const ProgressBar = () => {
    const percentage = Math.min(100, status.percentage || 0);
    const bgColor = 
      status.warningLevel === 'critical' ? 'bg-red-500' :
      status.warningLevel === 'warning' ? 'bg-yellow-500' : 
      status.warningLevel === 'notice' ? 'bg-blue-500' : 'bg-gray-500';
  ...
  const getMessage = () => {
    ...
    const percentageText = (status.percentage || 0).toFixed(0);
    const tokensRemaining = (status.tokensRemaining || 0).toLocaleString();
    ...
    switch (status.warningLevel) {
      ...
      case 'critical':
        return `Context limit almost reached (${percentageText}% used). ${tokensRemaining} tokens remaining.`;
      default:
        return `Context window ${percentageText}% used. ${tokensRemaining} tokens remaining.`;
    }
  };
  ...
  return (
    <div className={`text-sm p-2 rounded ${statusStyles[status.warningLevel || 'none']}`}>
      <div className="flex items-center">
        {statusIcons[status.warningLevel || 'none']}
